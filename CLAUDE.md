...
- RTK 판정은 `gps_quality` → `gps_sigma_m` → `fix_cov_xx` → `fix_status` 순으로
  **쓸 수 있는 열을 자동으로 고르고, 무엇으로 판정했는지 화면에 표시**한다.
  `fix_status`는 값이 한 종류뿐이면 쓰지 않는다 (드라이버가 RTK를 2로 뭉뚱그린다)
- RTK 고정은 밝게, 아니면 어둡게 — 지도와 오차 그래프 띠가 **같은 규칙**을 쓴다
...
- RTK: `fix_status`는 3,222행 전부 `2`라 판정 불가. `fix_cov_xx`가 σ 1.0~1.4 cm ↔ 2.1~2.6 m로
  갈리고 그 사이(5~50 cm)에 걸친 점이 0개 → 686/806점(85.1%)이 RTK 고정,
  비RTK는 t_rel 43.4~54.2 s와 84.6~97.4 s 두 구간(24.8 m)
- 비RTK 구간 평균 오차 0.25 m vs RTK 구간 0.18 m — 위치가 튀진 않았고 수신기가 정밀도를
  낮춰 보고한 것에 가깝다. 매핑 CSV에는 상태·정밀도 열이 아예 없다
